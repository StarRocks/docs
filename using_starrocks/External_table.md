...
To access HDFS data, you must create an external table in StarRocks for accessing HDFS files.
...
Table 1: Operators that support predicate pushdown
...
* Querying data from StarRocks may be much slower than directly querying data from Elasticsearch, such as count-related queries. The reason is that Elasticsearch directly reads the metadata of target documents without the need to filter the real data, which accelerates the count query.
